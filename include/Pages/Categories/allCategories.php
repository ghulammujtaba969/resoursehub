...
//------------------------------------------------
	$sqllms  = $dblms->querylms("SELECT *   
										FROM ".CATEGORIES." 
										WHERE cat_id != '' $sql2 
										ORDER BY cat_id DESC LIMIT ".($page-1)*$Limit .",$Limit");
//--------------------------------------------------
...
$srno++;
//-------------------------------------------------------------------------
	$ctystatus = get_admstatus($rowstd['cat_status']);
//-------------------------------------------------------------------------
echo'
<tr>
	<td> '.$rowstd['cat_id'].' </td>
	<td><h2><a> '.$rowstd['cat_name'].' </a></h2></td>
	<td> '.$rowstd['cat_abbr'].' </td>
	<td> '.$ctystatus.' </td>
	<td class="text-right">
		<div class="actions">
			<a href="Categories.php?view=modify&Cid='.$rowstd['cat_id'].'" class="btn btn-sm bg-success-light mr-2"><i class="fas fa-pen"></i></a>
			<a class="btn btn-sm bg-danger-light" href="Categories.php?Cid='.$rowstd['cat_id'].'&view=delete" onClick="javascript: return confirm(\'Please confirm deletion\');"> <i class="fas fa-trash"></i></a>
		</div>
	</td>
</tr>';
//------------------------------------------------
